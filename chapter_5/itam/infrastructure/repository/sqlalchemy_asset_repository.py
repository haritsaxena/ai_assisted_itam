...
class SQLAlchemyAssetRepository(BaseRepository[Asset]):
    ...
    def read(self, asset_id: int) -> Asset:
        ...
        a = Asset(id=asset.id, name=asset.name, status=asset.status, category=asset.category, cost=asset.cost, useful_life=asset.useful_life, salvage_value=asset.salvage_value, purchase_date=asset.purchase_date, locations=[])
